...
> 用途：在 `@repo/common` 中落地 `CodeMirrorMdEditor` 组件，提供 Markdown 编辑场景下的表达式输入、替换渲染与高亮渲染能力。  
> 适用：`packages/common/src/components/codemirror-md-editor/*`
...
- 应用层使用方式：仅通过 `@repo/common` 导入
- 禁止事项：禁止跨包相对路径直连源码内部实现
- 对外暴露入口：`packages/common/src/components/codemirror-md-editor/index.ts`
...
```txt
packages/common/src/components/codemirror-md-editor/
...
- 所属包：`packages/common/package.json`
- `name`：`@repo/common`
- `type`：`module`
...
- 目录：`apps/studio-web/src/components/playground/md-editor/`
